...
class IV(Analysis):
    ...
    def visualize(self, feat, name):
        iv_df, iv_value = self.calculate_iv(feat)
        fig, ax = plt.subplots(figsize=(10,6))
        sns.barplot(x=feat.feature, y=name, data=iv_df, \
            palette=sns.cubehelix_palette(len(iv_df.index), start=.5, rot=-.75, reverse=True))
        ax.set_title('{} visualization for: '.format(str(name).upper()) + feat.feature)
        plt.xticks(rotation=90)
        plt.show()
